...
module SY
  ...
  module UnitMethodsMixin
    ...
    def method_missing( method_ß, *args, &block )
      ...
      puts "units is class #{units.class}"
      unit_ςs = units.map { |u|
        [ u.name.downcase.to_s, # full unit symbols are in lower case !!!
          u.short.to_s ]
      }.reduce [], :+
      prefixes = PREFIX_TABLE.full + PREFIX_TABLE.short
      prefix_hash = PREFIX_TABLE.hash_full.merge( PREFIX_TABLE.hash_short )

      puts "unit_ςs is a #{unit_ςs.class}"
      prefixes, units, exponents = SPS_PARSER.( method_ß.to_s, unit_ςs, prefixes )
