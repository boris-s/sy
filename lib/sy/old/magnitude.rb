...
require_relative 'magnitude/absolute'
require_relative 'magnitude/signed'

# Magnitude generic class.
# 
class SY::Magnitude
  ★ ExpressibleInUnits             # ★ means include
  ★ Comparable
  ★ FlexCoerce
  ...
  class << self
    ...
    # Constructs zero magnitude of a given quantity.
    # 
    def zero( of: nil )
      absolute of: of, amount: 0
    end

    # Constructs magnitude 1 of a given quantity.
    # 
    def one( of: nil )
  ...
  # Expects quantity and amount (+:of+ and +:amount+ arguments).
  # 
  def initialize( of: nil, amount: nil )
    puts "Constructing magnitude of #{of}, amount: #{amount}" if SY::DEBUG
    fail ArgumentError, "Quantity not supplied!" if of.nil?
    @quantity = of
    @amount = case amount
              when Numeric then
                puts "amount is a Numeric, using it directly" if SY::DEBUG
                amount
              when nil then
                ...
                begin
                  ...
                  amount.reframe( quantity ).amount
  ...
  # Three-way comparison operator for magnitudes.
  # 
  def <=> m2
    return amount <=> m2.amount if
  ...
  # Reframes the magnitude into its relative quantity, making the amount
  # negative.
  # 
  def -@
    quantity.relative.magnitude( -amount )
  ...
  # Returns absolute value without any reframing.
  # 
  def abs
    magnitude amount.abs
  end

  # Returns a new magnitude with rounded amount.
  # 
  def round *args
    magnitude amount.round( *args )
  end

  # Attempts to reframe the magnitude into the provided quantity. Dimensions
  # must match.
  # 
  def reframe arg
    case arg
    when SY::Quantity then arg.import self
    when SY::Unit then arg.quantity.import( self ) # TODO: Here, it should go on like .prefer_unit( arg )
    else
      begin
        return reframe SY.Quantity( arg )
      rescue NameError, TypeError
        begin
          return reframe SY.Unit( arg )
        rescue NameError, TypeError
          fail TypeError, "Unable to reframe into #{arg}!"
        end
      end
    end
  end

  # Reframes the magnitude into a <em>relative</em> quantity. If the argument
  # would imply absolute quantity, its relative counterpart is used.
  # 
  def call arg
    case arg
    when SY::Quantity then arg.relative.import self
    when SY::Unit then arg.quantity.relative.import self # TODO: Dtto
    else
      begin
        return call SY.Quantity( arg )
      rescue NameError, TypeError
        begin
          return call SY.Unit( arg )
        rescue NameError, TypeError
          fail TypeError, "Unable to reframe into #{arg}!"
        end
      end
  ...
  def + m2
    ...
    return self + reframe( m2 ) if quantity.coerces? m2.quantity
    apply_through_coerce :+, m2
  end
  ...
  def * m2
    case m2
    when Numeric then magnitude amount * m2
    when Matrix then m2.map { |e| self * e }
    else
  ...
  def ** exp
    ...
    when SY::Magnitude then
      fail SY::DimensionError, "Exponent (#{exp}) not of zero dimension!" unless
        exp.dimension.zero?
  ...
  def in m2
    ...
    when SY::Magnitude then
      quantity.measure( from: m2.quantity ).t2r.( amount ) / m2.amount
    else
